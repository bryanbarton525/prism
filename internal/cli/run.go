...
list. Provide the task via --input <file> or --stdin; if neither flag is given
the command reads from stdin automatically.

Output is either JSON (default) or Markdown (--format markdown).

Examples:
  prism run github-cli --skills gh-pr-triage --input task.md
  echo "Check PR #42" | prism run github-cli --skills gh-pr-triage --stdin
  prism run web-docs-search --skills docs-source-harvest --format markdown --input task.md`,
...
// resolveTask reads the task text from --input, --stdin, or piped stdin.
func resolveTask(rf runFlags) (string, error) {
	...
	// --stdin flag or piped input (stdin is not a terminal).
...
// stdinIsPiped returns true when stdin is not an interactive terminal.
