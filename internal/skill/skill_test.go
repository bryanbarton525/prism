...
func TestParseFile_Valid(t *testing.T) {
	...
	sk, err := LoadDir(os.DirFS(root), "gh-pr-triage")
	if err != nil {
...
func TestParseFile_DescriptionTooLong(t *testing.T) {
	...
	_, err := LoadDir(os.DirFS(root), "gh-pr-triage")
	if err == nil {
...
body`,
	...
	skills, err := LoadMany(os.DirFS(root), []string{"gh-pr-triage", "kubectl-triage"})
	if err != nil {
...
func TestLoadMany_MissingSkill(t *testing.T) {
	root := t.TempDir()
	_, err := LoadMany(os.DirFS(root), []string{"nonexistent"})
	if err == nil {
...
func TestFullText_ContainsKeyParts(t *testing.T) {
	root := makeSkillDir(t, "gh-pr-triage", validSkillMD)
	sk, _ := LoadDir(os.DirFS(root), "gh-pr-triage")
...
func TestLoadMany_RealSkills(t *testing.T) {
	...
	skills, err := LoadMany(os.DirFS("../../skills"), []string{"gh-pr-triage", "kubectl-triage"})
	if err != nil {
